...
metadata$year <- "2011-12"
metadata$varName_harmonized <- colnames(plant_harvest)
metadata$dataset_name <- "ETH_2011_ERSS_v02_M"
metadata$dataset_doi <- "https://doi.org/10.48529/80xt-9m68"
...
metadata$varName_source=c(
  ...
  "ph_s9q13_a", #"harvest_month_begin",
  "ph_s9q13_b", #"harvest_month_end",
  "region", #"adm1",
  "zonename", #"adm2",
  "woredaname", #"adm3",  
  "LAT_DD_MOD", #"lat",
  "LON_DD_MOD", #"lon",
  NA, #"GPS_level",
  "pp_s3q02_d", #"plot_area_reported_localUnit",
  "pp_s3q02_c", #"localUnit_area",
  "pp_s3q05_c", #"plot_area_measured_ha",
...
metadata$varLabel_source=c(
  ...
  "What were the months when the harvest started and ended? (Start Month)", #"harvest_month_begin",
  "What were the months when the harvest started and ended? (End Month)", #"harvest_month_end",
  "Region", #"adm1",
  "Zone Name", #"adm2",
  "Woreda Name", #"adm3",
  ...
  NA, #"GPS_level",
  "What is the area of [Field]? (Total Area)", #"plot_area_reported_localUnit",
  "What is the area of [Field]? (Unit)",  #"localUnit_area",
  "GPS measured area of [Field] (Total Square Meters)", #"plot_area_measured",
  NA, #plot_area_reported_ha",
  NA, #"season",
  NA, #"harvest_month",
  NA, #"harvest_year",
...
metadata$comment=c(
  ...
  "range: 0 - 8000", #"planting_year",
  "The value 13 describes the 13th month of the Ethiopian calendar Pagume (6th September - 10th September/ 11th Sepember in leap years), values up to 23", #"harvest_month_begin",
  "The value 13 describes the 13th month of the Ethiopian calendar Pagume (6th September - 10th September/ 11th Sepember in leap years), values up to 23", #"harvest_month_end",
  "no region 14", #"adm1",
  NA, #"adm2",
  NA, #"adm3",  
  NA, #"lat",
...
zb <- zb %>% select(household_id, saq01, saq02, saq03) %>%
  ...
  mutate(woredaid = paste(regionid, zoneid, woredaid, sep = "_")) %>%
  mutate(zoneid = paste(regionid, zoneid, sep = "_")) 
zb <- left_join(b, regioncode, by = "regionid") 
...
zc <- zc %>% select(household_id, parcel_id, field_id, 
  ...
  set_names(c("hhID", "parcelID", "fieldID", 
...
zd <- zd %>% select(household_id, parcel_id, field_id, crop_name,
...
ze <- ze %>% select(household_id, LAT_DD_MOD, LON_DD_MOD) %>%
...
zf <- zf %>% select(household_id, parcel_id, field_id, 
...
write_csv(metadata, "out/ETH_2011-12_metadata.csv")
