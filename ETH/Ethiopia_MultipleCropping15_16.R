...
# Input files -------------------------------------------------------------
in.path.eth <- "L:/Data/LSMS/Ethiopia/LSMS_2015-2016_Ethiopia/Data/ETH_2015_ESS_v03_M_CSV/"
harvesting.file <- "sect9_ph_w3.csv" 
planting.file <- "sect4_pp_w3.csv" 
field.file <- "sect3_pp_w3.csv" #Field roster
parcel.file <- "sect10_hh_w3.csv" #Land Parcel roster, unsused
householdidentification.file <- "sect_cover_hh_w3.csv" #Household Identification Particulars
geovariables.file <- "ETH_HouseholdGeovars_y3.csv" #Household GPS coordinates
...
d <- d %>% select(household_id2, parcel_id, field_id, crop_name,
...
f <- f %>% select(household_id2, parcel_id, field_id, pp_s3q02_a, pp_s3q02_c, pp_s3q05_a, holder_id) %>%
...
plant_harvest$dataset_name <- "ETH_2015_ESS_v03_M" #the online version changed to v03_M ??
plant_harvest$country <- "Ethiopia"
plant_harvest$dataset_doi <- "https://doi.org/10.48529/ampf-7988"
...
# Write output table ------------------------------------------------------
write_csv(plant_harvest, "out/ETH_2015-16.csv")
