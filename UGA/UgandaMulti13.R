...
# Input files -------------------------------------------------------------
in.path.uga <- "UGA_2013_UNPS_v02_M_CSV/"
...
householdidentification.file <- "agsec1.csv" #Household Identification Particulars
crop.code <- 'Crop_Code.csv'
district.code <- 'district_Code.csv'
...
district_code <- read_csv(paste0(in.path.uga, district.code)) %>%
  set_names(c('district_code', 'adm1'))
...
b <- b %>% select(HHID, region, district_name, subcounty_name, parish_name) %>%
...
c <- c %>% select(HHID, plotID, cropID,  
                  a4aq7, a4aq9, a4aq9_1, a4aq9_2,a4aq8) %>%
  ...
  mutate(planting_year = planting_year + 1999) %>%
...
c$crop_area_share[(is.na(c$crop_area_share)) & (c$crop_stand==1)]=100 #fill NA is crop_area_share if pure stand
...
d <- d %>% select(HHID, plotID, cropID, 
  ...
  filter(harvest_year_begin %in% c(2001:2017)) %>%
  filter(harvest_year_end %in% c(2001:2017))
...
e <- e %>% select(HHID, plotID, cropID,  
                  a4bq7, a4bq9, a4bq9_1, a4bq9_2,a4bq8) %>%
  set_names(c("hhID", "plotID", "cropID", "plot_area_reported_localUnit",
              "crop_area_share", "planting_month", "planting_year","crop_stand")) %>%
 # mutate(plot_area_reported = plot_area_reported / 2.471) %>% #acres to hectares 
  #mutate(plot_area_reported = plot_area_reported * (if_else(is.na(crop_perc), 1, crop_perc/100)), .keep='unused') %>%
...
e$crop_area_share[(is.na(e$crop_area_share)) & (e$crop_stand=="Pure Stand")]=100 #fill NA is crop_area_share if pure stand
...
plant_harvest$source <- "UGA_2013_UNPS_v02_M"
plant_harvest$dataset_doi <- "https://doi.org/10.48529/c1c4-h654"
...
# Write output table ------------------------------------------------------
write_csv(plant_harvest, "out/uganda13-14.csv")
