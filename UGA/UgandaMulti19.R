...
# Input files -------------------------------------------------------------
in.path.uga <- "L:/Data/LSMS/Uganda/LSMS_2019-20_Uganda/UGA_2019_UNPS_v03_M_CSV/UGA_2019_UNPS_v03_M_CSV/"
...
b <- b %>% select(hhid, district, s1aq03a, s1aq04a) %>%
...
c <- c %>% select(hhid, pltid, cropID,  
  ...
  mutate(join_ID = paste(fieldID, plotID, cropID, sep = "_"))
...
plant_harvest_first <- full_join(c, d, by=c("hhID", "plotID", "crop", "join_ID", "fieldID"))
plant_harvest_first <- plant_harvest_first %>%
  mutate(season = "first") %>% 
  select(!c(cropID.x, cropID.y, join_ID)) #remove crop ID as crop name is now included

# Create data.frame for second visit ---------------------------------------------
plant_harvest_second <- full_join(e, f, by=c("hhID", "plotID", "crop", "join_ID", "fieldID"))
...
plant_harvest$dataset_name <- "UGA_2019_UNPS_v03_M"
plant_harvest$country <- "Uganda"
plant_harvest$dataset_doi <- "https://doi.org/10.48529/nqzx-f196"
...
# Write output table ------------------------------------------------------
write_csv(plant_harvest, "out/UGA_2019-20.csv")
